...
0;l<h.length;l+=1)this.vb(h[l],l+1);return!0};a.prototype.reset=function(){return 0===Ac(this.Ra)&&0===zc(this.Ra)};a.prototype.freemem=function(){for(var h;void 0!==(h=this.lb.pop());)oa(h)};a.prototype.free=function(){var h=0===Bc(this.Ra);delete this.db.fb[this.Ra];this.Ra=0;return h};b.prototype.next=function(){if(null===this.eb)return{done:!0};null!==this.$a&&(this.$a.free(),this.$a=null);if(!this.db.db)throw this.nb(),Error("Database closed");var h=pa(),l=x(4);qa(d);qa(l);try{this.db.handleError(ba(this.db.db,
this.jb,-1,d,l));this.jb=v(l,"i32");var q=v(d,"i32");if(0===q)return this.nb(),{done:!0};this.$a=new a(q,this.db);this.db.fb[q]=this.$a;return{value:this.$a,done:!1}}catch(p){throw this.pb=A(this.jb),this.nb(),p;}finally{ra(h)}};b.prototype.nb=function(){oa(this.eb);this.eb=null};b.prototype.getRemainingSQL=function(){return null!==this.pb?this.pb:A(this.jb)};"function"===typeof Symbol&&"symbol"===typeof Symbol.iterator&&(b.prototype[Symbol.iterator]=function(){return this});c.prototype.run=function(h,
l){if(!this.db)throw"Database closed";if(l){h=this.prepare(h,l);try{h.step()}finally{h.free()}}else this.handleError(t(this.db,h,0,0,d));return this};c.prototype.exec=function(h,l,q){if(!this.db)throw"Database closed";var p=pa(),z=null;try{var M=aa(h)+1,G=x(M);k(h,y,G,M);var ja=G;var ca=x(4);for(h=[];0!==v(ja,"i8");){qa(d);qa(ca);this.handleError(ba(this.db,ja,-1,d,ca));var D=v(d,"i32");ja=v(ca,"i32");if(0!==D){M=null;z=new a(D,this);for(null!=l&&z.bind(l);z.step();)null===M&&(M={columns:z.getColumnNames(),
...
function(h){return new b(h,this)};c.prototype["export"]=function(){Object.values(this.fb).forEach(function(l){l.free()});Object.values(this.Xa).forEach(sa);this.Xa={};this.handleError(n(this.db));var h=ta(this.filename);this.handleError(g(this.filename,d));this.db=v(d,"i32");return h};c.prototype.close=function(){null!==this.db&&(Object.values(this.fb).forEach(function(h){h.free()}),Object.values(this.Xa).forEach(sa),this.Xa={},this.handleError(n(this.db)),ua("/"+this.filename),this.db=null)};c.prototype.handleError=
function(h){if(0===h)return null;h=rc(this.db);throw Error(h);};c.prototype.getRowsModified=function(){return w(this.db)};c.prototype.create_function=function(h,l){Object.prototype.hasOwnProperty.call(this.Xa,h)&&(sa(this.Xa[h]),delete this.Xa[h]);var q=va(function(p,z,M){for(var G,ja=[],ca=0;ca<z;ca+=1){var D=v(M+4*ca,"i32"),N=Dc(D);if(1===N||2===N)D=Hc(D);else if(3===N)D=Fc(D);else if(4===N){N=D;D=Ec(N);N=Gc(N);for(var xb=new Uint8Array(D),Ba=0;Ba<D;Ba+=1)xb[Ba]=y[N+Ba];D=xb}else D=null;ja.push(D)}try{G=
l.apply(null,ja)}catch(Oc){ub(p,Oc,-1);return}switch(typeof G){case "boolean":Lc(p,G?1:0);break;case "number":Ic(p,G);break;case "string":Jc(p,G,-1,-1);break;case "object":null===G?tb(p):null!=G.length?(z=na(G),Kc(p,z,G.length,-1),oa(z)):ub(p,"Wrong API use : tried to return a value of an unknown type ("+G+").",-1);break;default:tb(p)}});this.Xa[h]=q;this.handleError(Cc(this.db,h,l.length,1,0,q,0,0,0));return this};e.Database=c};var wa={},B;for(B in e)e.hasOwnProperty(B)&&(wa[B]=e[B]);
var xa="./this.program",ya=!1,za=!1,Aa=!1,Ca=!1;ya="object"===typeof window;za="function"===typeof importScripts;Aa="object"===typeof process&&"object"===typeof process.versions&&"string"===typeof process.versions.node;Ca=!ya&&!Aa&&!za;var E="",Da,Ea,Fa,Ga,Ha;
